...
export class BSCMonitor {
  ...
  async start(onNewContract: (contract: NewContract) => void): Promise<void> {
    console.log('[BSC] Starting monitor...');

    if (this.startBlock === 0) {
      this.currentBlock = await this.provider.getBlockNumber() - 10;
    }

    console.log(`[BSC] Starting from block: ${this.currentBlock}`);
  ...
  private async poll(onNewContract: (contract: NewContract) => void): Promise<void> {
    try {
      ...
      console.error('[BSC] Error polling:', error);
    }
    ...
    try {
      ...
      for (const tx of block.transactions) {
        ...
        if (txObj.to === null && txObj.creates) {
          ...
          console.log(`[BSC] New contract: ${contractAddress}${contract.tokenInfo ? ` (${contract.tokenInfo.symbol})` : ''}`);
          onNewContract(contract);
        }
      }
    } catch (error) {
      console.error(`[BSC] Error checking block ${blockNum}:`, error);
    }
  }
